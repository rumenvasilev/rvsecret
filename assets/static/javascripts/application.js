let Stats = Backbone.Model.extend({
    url: "/api/stats",
    defaults: {
        "Status": "initializing",
        "StartedAt": null,
    ...
    duration: function () {
        ...
        let end;
        let start = Date.parse(this.get("StartedAt"));
        if (this.isFinished()) {
...
let Finding = Backbone.Model.extend({
    idAttribute: "Id",
    ...
    trimmedCommitMessage: function () {
        let message = this.get("CommitMessage").split("-----END PGP SIGNATURE-----", 2).pop();
        return message.replace(/^\s\s*/, "").replace(/\s\s*$/, "")
    },
    isTestRelated: function () {
        let path = this.get("FilePath").toLowerCase();
        for (let i = 0; i < this.testFileIndicators.length; i++) {
    ...
    fileContentsUrl: function () {
        return ["/api/files", this.get("RepositoryOwner"), this.get("RepositoryName"), this.get("CommitHash"), this.get("FilePath")].join("/");
    },
...
let Findings = Backbone.Collection.extend({
    url: "/api/findings",
    model: Finding,
});
...
let FindingView = Backbone.View.extend({
    tagName: "tr",
    events: {
...
let FindingsView = Backbone.View.extend({
    collection: findings,
    initialize: function () {
        ...
        $("#finding_modal").on("show.bs.modal", function (event) {
            $(document).on("keydown", function (e) {
                ...
                switch (e.keyCode) {
                    case 37:
                        finding = findingsView.previousFinding();
                        break;
                    case 39:
                        finding = findingsView.nextFinding();
                        break;
                    default:
                        return;
                }
